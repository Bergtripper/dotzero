...
export const Hero: React.FC<HeroProps> = ({ onNavigateToModulor, onNavigateToMusa }) => {
  ...
  return (
    <section
      id="hero-section"
      className="relative pt-24 sm:pt-28 pb-16 sm:pb-24 border-b-2 border-[var(--text)] overflow-hidden"
    >
        ...
        {navigateToSubpage && (
          <div className="mb-8 border-2 border-[var(--text)] bg-[#07080C] text-[var(--on-accent)] p-3 sm:p-4 shadow-[4px_4px_0px_0px_#DE3831] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[11px] font-mono text-[var(--accent-tertiary)] uppercase tracking-widest font-bold block">
                  {tModulor.badge[language]}
                </span>
                <span className="text-sm sm:text-base font-bold text-[var(--on-accent)] tracking-wide">
            ...
            <button
              ...
              className="px-4 py-2 bg-[var(--accent)] text-[var(--on-accent)] font-mono text-xs uppercase tracking-wider font-bold hover:bg-[var(--accent)]/90 flex items-center justify-center gap-2 self-start sm:self-auto transition-all shadow-[2px_2px_0px_0px_#F7B801]"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col justify-between">
            ...
            <div className="flex items-center gap-3 mb-4">
              <span className="inline-block w-4 h-4 bg-[var(--accent)] border border-[var(--text)]" />
              <span className="font-mono text-xs uppercase tracking-[0.2em] font-semibold text-[var(--text)]">
                {t.badge[language]}
              </span>
            </div>
            ...
            >
              ...
              <span className="text-[var(--accent)] relative inline-block">
                ZERO
                <span className="absolute -bottom-1 left-0 right-0 h-3 bg-[var(--accent-tertiary)] -z-10 transform skew-x-[-12deg]" />
              </span>
            </h1>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-4">
              ...
              <a
                href="#laboratorio"
                id="hero-cta-lab"
                className="px-6 py-3.5 bg-[var(--surface-raised)] text-[var(--text)] font-mono text-xs uppercase tracking-widest font-bold border-2 border-[var(--text)] shadow-[4px_4px_0px_0px_#1350B0] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_#1350B0] transition-all flex items-center gap-2"
              >
            ...
            <div className="mt-12 pt-6 border-t-2 border-[var(--text)] grid grid-cols-3 gap-4 font-mono">
              <div className="border-r border-[var(--text)] pr-2">
                <div className="text-2xl font-bold text-[var(--text)]">1919</div>
                <div className="text-[11px] text-zinc-600 uppercase tracking-wider">
                  {t.metricOriginLabel[language]}
                </div>
              </div>
              <div className="border-r border-[var(--text)] pr-2">
                <div className="text-2xl font-bold text-[var(--accent-secondary)]">0.00%</div>
                <div className="text-[11px] text-zinc-600 uppercase tracking-wider">
                  {t.metricOrnamentLabel[language]}
                </div>
              </div>
              <div>
                <div className="text-2xl font-bold text-[var(--accent)]">3 / 3</div>
                <div className="text-[11px] text-zinc-600 uppercase tracking-wider">
          ...
          <div className="lg:col-span-5 flex flex-col items-center">
            <div className="w-full max-w-md bg-[var(--surface-raised)] border-2 border-[var(--text)] p-5 shadow-[8px_8px_0px_0px_#121212] relative">
              
              {/* Sculpture Header Controls */}
              <div className="flex items-center justify-between pb-3 mb-3 border-b-2 border-[var(--text)] font-mono text-xs">
              ...
              >
                ...
                <div className="absolute top-1/2 left-0 right-0 h-[1px] bg-[var(--text)]/20 border-dashed" />
                <div className="absolute left-1/2 top-0 bottom-0 w-[1px] bg-[var(--text)]/20 border-dashed" />
                ...
                >
                  ...
                  <motion.div
                    className="absolute w-36 h-36 bg-[var(--accent-secondary)] border-2 border-[var(--text)]"
                    animate={{
                  ...
                  <motion.div
                    className="absolute w-32 h-32 rounded-full bg-[var(--accent)] border-2 border-[var(--text)] mix-blend-multiply"
                    animate={{
                  ...
                  <motion.div
                    className="absolute w-60 h-3 bg-[var(--text)] z-20"
                    animate={{
                  ...
                  <div className="absolute w-4 h-4 rounded-full bg-[var(--surface-raised)] border-2 border-[var(--text)] z-30 flex items-center justify-center">
                    <span className="w-1.5 h-1.5 bg-[var(--accent)] rounded-full" />
                  </div>
                </motion.div>

                {/* Overlay Technical Labels */}
                <div className="absolute bottom-2 left-3 font-mono text-[10px] text-zinc-600 bg-[var(--surface-raised)]/80 px-1.5 py-0.5 border border-[var(--text)]">
                  {t.sculptureHint[language]}
                </div>
                <div className="absolute top-2 right-3 font-mono text-[10px] text-zinc-600 bg-[var(--surface-raised)]/80 px-1.5 py-0.5 border border-[var(--text)]">
                  {t.sculptureState[language]}: 0{sculptureState + 1} / 04
                </div>
              </div>

              {/* Bottom Interactive Primitives Palette */}
              <div className="mt-3 pt-3 border-t-2 border-[var(--text)] flex items-center justify-between text-xs font-mono">
                <span className="text-zinc-600">{t.pureElements[language]}</span>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setRotation((r) => r + 90)}
                    className="flex items-center gap-1 px-2 py-0.5 border border-[var(--text)] hover:bg-[var(--accent)] hover:text-[var(--on-accent)] transition-colors"
                  >
                    <span className="w-2.5 h-2.5 rounded-full bg-[var(--accent)] inline-block border border-[var(--text)]" />
                    <span>{t.circle[language]}</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => setRotation((r) => r + 180)}
                    className="flex items-center gap-1 px-2 py-0.5 border border-[var(--text)] hover:bg-[var(--accent-secondary)] hover:text-[var(--on-accent)] transition-colors"
                  >
                    <span className="w-2.5 h-2.5 bg-[var(--accent-secondary)] inline-block border border-[var(--text)]" />
                    <span>{t.square[language]}</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => setRotation((r) => r + 45)}
                    className="flex items-center gap-1 px-2 py-0.5 border border-[var(--text)] hover:bg-[var(--accent-tertiary)] transition-colors"
                  >
